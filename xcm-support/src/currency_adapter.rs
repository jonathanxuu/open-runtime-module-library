	...
	>
{
	fn deposit_asset(asset: &MultiAsset, location: &MultiLocation) -> Result {
		match (
			...
			// known asset
			(who, Some(currency_id), Some(amount)) => {
				MultiCurrency::deposit(currency_id, &who.unwrap(), amount).map_err(|e| XcmError::FailedToTransactAsset(e.into()))
			}
			// unknown asset
	...
	fn withdraw_asset(asset: &MultiAsset, location: &MultiLocation) -> result::Result<xcm_executor::Assets, XcmError> {
